...
function displayOrders(orders) {
    ...
    orders.forEach(order => {
        ...
        productDiv.onclick = () => {
            showDetails(order.orderId, order.prodDetails[0].imgurl, order.prodDetails[0].name, order.prodDetails[0].description, order.orderPrice, order.quantity, order.status, order.orderTime, order.phoneNo, order.Address, order.username);
        };
...
function showDetails(orderId, imgurl, prodName, prodDesc, proPrice, proQuan, Ordstatus, orderDate, phoneNo, Address, userName) {
